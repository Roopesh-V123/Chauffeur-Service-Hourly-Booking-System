...
reportsRouter.get("/summary", async (req: Request, res: Response) => {
  try {
    ...
    const bookings = await prisma.chauffeurServiceHourlyBooking.findMany({
      where: whereClause,
      include: {
        vehicle: true,
        payments: true
    ...
    bookings.forEach((booking) => {
      ...
      try {
        const bill = calculateBookingBill(
          booking.id,
    ...
    const timeSeriesMap: { [dateStr: string]: { bookings: number; revenue: number; hours: number } } = {};
