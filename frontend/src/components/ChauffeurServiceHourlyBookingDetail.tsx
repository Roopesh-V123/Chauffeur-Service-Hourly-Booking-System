  ...
  if (!booking) {
    return (
      <div className="bg-surface border border-border-color/30 rounded-2xl p-12 shadow-lg text-center text-foreground">
        ...
        <button
          onClick={onBack}
          className="px-6 py-2.5 bg-accent-active text-white rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-accent-active/85 transition-colors"
        >
          Return to Monitor
        </button>
      </div>
    ...
    <div className="bg-surface border border-border-color/30 rounded-2xl shadow-xl p-8 max-w-full lg:max-w-5xl mx-auto transition-all duration-300 print:shadow-none print:border-none print:p-0 text-foreground">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-border-color/30 pb-6 mb-8 gap-4 print:mb-6">
        <div>
          <button
            onClick={onBack}
            className="text-xs font-black text-muted hover:text-accent-active uppercase tracking-wider mb-2 flex items-center gap-1.5 transition-colors print:hidden"
          >
          ...
          <p className="text-[10px] text-muted font-black uppercase tracking-wider mt-2">
            Reference ID: <span className="font-mono text-accent-active">{booking.id}</span>
          </p>
        </div>
      ...
      {errorState && (
        <div className="bg-amber-950/40 border-l-4 border-amber-600 text-amber-200 p-4 rounded-r-lg mb-8 text-xs font-semibold print:hidden">
          ⚠️ {errorState}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-8">
          ...
          <div className="bg-background border border-border-color/20 rounded-xl p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-6 text-xs font-semibold">
              <div>
                <span className="text-muted block mb-1">Meter Hours Status</span>
                <span className="text-lg font-black text-accent-active">₹{booking.liveMeterAndBilling.toLocaleString()}</span>
              </div>
              <div>
                <span className="text-muted block mb-1">Status Classification</span>
          ...
          <div className="bg-background border border-border-color/20 rounded-xl p-6">
            ...
            {booking.customer ? (
              ...
              <div className="text-center py-4 text-muted text-xs font-bold uppercase tracking-wider">
            ...
            {booking.vehicle ? (
              <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 text-xs font-semibold">
                ...
                <div>
                  ...
                  <span className="text-accent-active">{booking.vehicle.category}</span>
                </div>
              </div>
            ) : (
              <div className="text-center py-4 text-muted text-xs font-bold uppercase tracking-wider">
        ...
        <div className="space-y-8">
            ...
            {booking.payments && booking.payments.length > 0 ? (
              <div className="space-y-4">
                {booking.payments.map((p) => (
                  <div
                    key={p.id}
                    className="bg-surface border border-border-color/30 rounded-lg p-3 text-xs font-semibold flex flex-col justify-between space-y-1.5"
                  >
                    <div className="flex justify-between items-center">
                      <span className="text-muted font-mono text-[9px] uppercase">ID: {p.id.slice(0,8)}</span>
              ...
              <div className="text-center py-8 text-muted text-xs font-bold uppercase tracking-wider">
          ...
          <div className="bg-background border border-border-color/20 rounded-xl p-6">
            ...
            <div className="relative border-l-2 border-border-color/20 ml-2.5 pl-6 space-y-6 text-xs font-semibold">
              {/* Event 1 */}
              <div className="relative">
                <div className="absolute -left-[31px] top-1 bg-accent-active w-2 h-2 rounded-full border-2 border-accent-active" />
              ...
              {/* Event 2 */}
              <div className="relative">
                <div className="absolute -left-[31px] top-1 bg-[#00b057] w-2 h-2 rounded-full border-2 border-[#00b057]" />
                <span className="text-muted text-[9px] block">
              ...
              {/* Event 3 */}
              <div className="relative">
                <div className="absolute -left-[31px] top-1 bg-muted w-2 h-2 rounded-full border-2 border-muted" />
                <span className="text-muted text-[9px] block">
      ...
      {/* Sheet Footer */}
      <div className="mt-8 border-t border-border-color/20 pt-6 text-center text-[10px] text-muted font-bold uppercase tracking-wider">
